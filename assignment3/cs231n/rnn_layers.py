...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    x_wx = x.dot(Wx)#N*H
    prev_wh = prev_h.dot(Wh)#N*H
    out = x_wx + prev_wh + b#H
    next_h = np.tanh(out)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N,H = h0.shape 
    h = np.zeros((N,T,H))
    hi = h0 # h0 is the affline lair from image feaures
    cache = []
    for i in range(T):#for every ts
        xi = x[:,i,:].reshape(N,D)
        hi,cachei = rnn_step_forward(xi, hi, Wx, Wh, b)#calc h(ts)
        h[:,i,:] = hi#assign h(ts)
        cache.append(cachei)
...
def rnn_backward(dh, cache):
    ...
    _,D = dxl.shape
    dx = np.zeros((N,T,D))
    dx[:,T-1,:] = dxl#update last state with it's grad
    for i in range(T-2, -1, -1):#go backwards till the end
        #run all the prev backwards steps
...
def word_embedding_backward(dout, cache):
    ...
    a
    array([2, 3, 5, 4]
    source : https://numpy.org/doc/stable/reference/generated/numpy.ufunc.at.html
    ...
    x, W = cache
    dW = np.zeros(W.shape)
    np.add.at(dW, x, dout) #add to dW (at indeces specified at x the value dout
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
